...
export default function GridGameCanvas({ 
  ...
  const gameStateRef = useRef<GridGameState | null>(null);
  ...
  useEffect(() => {
    // Character change should trigger a re-render of the canvas
    if (canvasRef.current) {
      // Just trigger a re-render by setting a dummy state change
  ...
  const triggerCelebration = () => {
    ...
    for (let i = 0; i < 30; i++) {
      ...
      particlesRef.current.push({
        ...
        life: 0,
        maxLife: 4000, // Longer celebration
        color: `hsl(${Math.random() * 360}, 80%, 60%)`,
        size: 25 + Math.random() * 15, // Bigger particles
        emoji: candyEmojis[Math.floor(Math.random() * candyEmojis.length)]
      });
    }
    
    // Additional burst from corners
    const corners = [
      { x: 50, y: 50 },
      { x: GRID_WIDTH * TILE_SIZE - 50, y: 50 },
      { x: 50, y: GRID_HEIGHT * TILE_SIZE - 50 },
      { x: GRID_WIDTH * TILE_SIZE - 50, y: GRID_HEIGHT * TILE_SIZE - 50 }
    ];
    ...
    screenShakeRef.current = { t: 800, mag: 12 }; // Stronger shake
  };
  ...
  const resetGame = () => {
    ...
    setIsGamePaused(false); // Reset pause state for new level
  };
  ...
  const drawGame = () => {
    ...
    // Draw exit (chest)
    const exitCenter = getTileCenter(currentGameState.exitPosition.x, currentGameState.exitPosition.y);
    ctx.font = '48px Arial';
    ctx.textAlign = 'center';
    ctx.fillText('📦', exitCenter.x, exitCenter.y);

    // Draw player
    const playerCenter = displayPosRef.current ?? getTileCenter(currentGameState.playerPosition.x, currentGameState.playerPosition.y);
    ctx.font = '40px Arial';
    ctx.textAlign = 'center';
    const emoji = selectedCharacter.emoji; // Use the actual character emoji
    ...
    ctx.fillText(`Keys: ${currentGameState.keysCollected}`, 20, 50);
  ...
  const movePlayer = (direction: 'up' | 'down' | 'left' | 'right') => {
    // Don't allow movement if game is paused or level is complete
    if (isGamePaused || gameState.levelComplete) {
      return;
    }
    ...
    setGameState(prev => {
      ...
      if (newX === newState.exitPosition.x && newY === newState.exitPosition.y) {
        ...
        if (allCoinsCollected) {
          ...
          // Notify parent after 5 seconds (let celebration play out)
          setTimeout(() => {
            onLevelComplete?.();
          }, 5000);
        }
    ...
    // Notify parent of state change after move
    setTimeout(() => {
      if (onGameStateChange) {
        onGameStateChange({ ...gameStateRef.current!, timeRemaining });
      }
    }, 0);
  };

  // Command execution
  // Reset execution when commandKey changes (new command execution)
  useEffect(() => {
    if (commandKey > 0) {
      setIsExecuting(false);
    }
  ...
  // Execute commands
  useEffect(() => {
    if (commands.length > 0 && !isExecuting && !gameState.levelComplete && !isGamePaused) {
      setIsExecuting(true);
  ...
  }, [commands, isExecuting, gameState.levelComplete, isGamePaused, onCommandComplete]);
  ...
  useEffect(() => {
    const loop = (now: number) => {
      ...
      if (!displayPosRef.current && gameStateRef.current) {
        const c = getTileCenter(gameStateRef.current.playerPosition.x, gameStateRef.current.playerPosition.y);
  ...
  }, []); // Remove gameState dependency to prevent unnecessary re-renders
